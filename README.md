...
- Service: use an endlessly extending, month-grouped all-days timeline (or switch to a compact Sunday-only timeline), and use the adjacent arrows to move between actual planned-service dates rather than fixed weekly offsets. Choosing an empty calendar date opens a deliberate Prepare service dialog without creating data; it preselects the schedule-matched type, previews its outline, and allows an exception type before confirmation. Creating a worship set also ensures the matching scheduled service and outline exist, reflecting the normal worship-first planning workflow. The compact type control on an existing service reopens setup so the type can be corrected while preserving content and adding missing sections. Administrators can create custom named service types, define each type's ordered default outline, and configure reusable weekly schedules by service type and weekday, including Welcome, countdown, service-start, and cleanup times. Outline sections are compact colour-coded containers which can contain several named, locally reorderable items such as sermon decks, announcement images, videos, or decks; groups reorder only among groups. Each child item is independently expandable in the rail and sorter. One group-level add button opens the item dialog, defaults Sermon and Announcements to Slide Deck, and includes an Images tab for creating one sortable item from one or more uploaded images. Presenters cannot move or remove template outline groups. Pre-service audio fades out when the countdown ends or service starts, while the final photo fades back to the LCF background. Service start changes to the neutral LCF holding background; worship begins only when the operator advances. Administrators can use the Start menu to rehearse or stop the service flow on demand.
