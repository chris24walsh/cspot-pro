...
- Service: use an endlessly extending, month-grouped all-days timeline (or switch to a compact Sunday-only timeline), open slots without a separate create step, build the running order, add songs, Bible passages, and slide decks, and control the live output. New Sunday services receive a timed, editable running-order scaffold from pre-service fellowship through post-service fellowship; older plans can add any missing standard sections without replacing existing content.
