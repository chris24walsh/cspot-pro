...
- Service: use an endlessly extending, month-grouped all-days timeline (or switch to a compact Sunday-only timeline), open slots without a separate create step, build the running order, add songs, Bible passages, and slide decks, and control the live output. New Sunday services receive a compact seven-section running order. Its 10:30 Welcome combines the pre-service montage, music, and countdown; Church Family provides a neutral shared transition; and Announcements and fellowship accepts an announcements deck directly. Older plans can add missing standard sections without replacing existing content.
