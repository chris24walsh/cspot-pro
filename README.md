...
- Service: use an endlessly extending, month-grouped all-days timeline (or switch to a compact Sunday-only timeline), select a service type beside the calendar/history controls, open slots without a separate create step, build the running order, add songs, Bible passages, and slide decks, and control the live output. Administrators can create custom named service types, define each type's ordered default outline, and configure reusable weekly schedules by service type and weekday, including Welcome, countdown, service-start, and cleanup times. Opening a date creates an empty plan; Add outline or creation of that date's worship set applies the schedule-matched type's template. Sunday, midweek, and other events can therefore have independent structures and automation. Presenters cannot move or remove template outline slides; sermon decks attach to and detach from a persistent Sermon slide. Pre-service audio fades out when the countdown ends or service starts, while the final photo fades back to the LCF background. Service start changes to the neutral LCF holding background; worship begins only when the operator advances. Administrators can use the Start menu to rehearse or stop the service flow on demand. Empty Sermon and Announcements placeholders open the deck search directly.
