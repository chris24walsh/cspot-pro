          ...
          <div className={`plan-type-outline-row ${item.parent_id ? "is-child" : "is-section"} ${draggedOutlineId === item.id ? "is-dragging" : ""}`} key={item.id ?? `${index}:${item.sequence}`} onDragOver={(event) => { if (draggedOutlineId) event.preventDefault(); }} onDrop={() => dropOutline(index)}>
            ...
            {(item.parent_id || expandedSectionSettings.has(item.id ?? String(index))) ? <div className={`template-cue-controls ${item.parent_id ? "" : "is-section-controls"}`}>
              ...
              {item.presentation_options?.overlay_mode === "countdown" ? <>
                {["welcome_montage", "welcome_countdown"].includes(item.item_type) ? <><label>Countdown target<select onChange={(event) => updateOutline(index, { presentation_options: { ...item.presentation_options, overlay_countdown_until: event.target.value === "until" ? item.presentation_options?.overlay_countdown_until || draft.starts_at || "11:00" : "" } })} value={item.presentation_options?.overlay_countdown_until ? "until" : "service"}><option value="service">Service start</option><option value="until">Clock time</option></select></label>{item.presentation_options?.overlay_countdown_until ? <label>Until<input onChange={(event) => updateOutline(index, { presentation_options: { ...item.presentation_options, overlay_countdown_until: event.target.value } })} type="time" value={item.presentation_options.overlay_countdown_until} /></label> : null}<label>Maximum countdown <span className="field-unit">seconds</span><input min={1} onChange={(event) => updateOutline(index, { presentation_options: { ...item.presentation_options, overlay_countdown_seconds: Number(event.target.value) } })} type="number" value={item.presentation_options?.overlay_countdown_seconds ?? (item.item_type === "welcome_montage" ? 1800 : 300)} /></label></> : <><label>Countdown<select onChange={(event) => updateOutline(index, { presentation_options: { ...item.presentation_options, overlay_countdown_until: event.target.value === "until" ? item.presentation_options?.overlay_countdown_until || "11:00" : "" } })} value={item.presentation_options?.overlay_countdown_until ? "until" : "time"}><option value="time">Time (duration)</option><option value="until">Until (clock time)</option></select></label>{item.presentation_options?.overlay_countdown_until ? <label>Until<input onChange={(event) => updateOutline(index, { presentation_options: { ...item.presentation_options, overlay_countdown_until: event.target.value } })} type="time" value={item.presentation_options.overlay_countdown_until} /></label> : <label>Time <span className="field-unit">seconds</span><input min={1} onChange={(event) => updateOutline(index, { presentation_options: { ...item.presentation_options, overlay_countdown_seconds: Number(event.target.value) } })} type="number" value={item.presentation_options?.overlay_countdown_seconds ?? 300} /></label>}</>}
              </> : null}
              ...
              {item.presentation_options?.auto_advance && item.item_type !== "welcome_countdown" ? <label>Advance after <span className="field-unit">seconds</span><input min={1} onChange={(event) => updateOutline(index, { presentation_options: { ...item.presentation_options, auto_advance_seconds: Number(event.target.value) } })} type="number" value={item.presentation_options?.auto_advance_seconds ?? item.presentation_options?.dwell_seconds ?? 12} /></label> : null}
              {item.item_type === "welcome_countdown" ? <p>Starts fresh on entry, capped by the original Welcome countdown. Auto-advance follows countdown zero.</p> : null}
