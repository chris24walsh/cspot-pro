...
function makeDrafts(data: ServingProfile): Record<string, ServingDraft> {
  return Object.fromEntries(data.areas.map((area) => {
    ...
    return [area.key, { selected: Boolean(preference), frequency_count: preference?.frequency_count ?? 1, frequency_period: preference?.frequency_period ?? "month", availability_notes: preference?.availability_notes ?? "" }];
  }));
}
