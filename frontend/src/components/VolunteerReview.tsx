...
interface ReviewDraft { status: VolunteerStatus; frequency_count: number; frequency_period: VolunteerFrequencyPeriod; admin_notes: string; }
const makeDraft = (row: VolunteerAdminRecord): ReviewDraft => ({ status: row.preference.status, frequency_count: row.preference.frequency_count, frequency_period: row.preference.frequency_period, admin_notes: row.preference.admin_notes ?? "" });
...
function AdminVolunteerRow({ autoFocus = false, confirm, expanded, onChanged, onToggle, row }: { autoFocus?: boolean; confirm: ConfirmAction; expanded: boolean; onChanged: () => Promise<void>; onToggle: () => void; row: VolunteerAdminRecord }) {
  ...
  async function persist(next: ReviewDraft) {
    ...
    setSaving(true);
    try {
      await reviewVolunteerPreference(row.preference.id, { status: next.status, frequency_count: next.frequency_count, frequency_period: next.frequency_period, admin_notes: next.admin_notes || null });
      await onChanged();
  ...
  return <article className={`compact-serving-role admin-serving-role ${row.preference.admin_attention_pending ? "is-pending" : ""}`} ref={rowRef}>
    ...
    {expanded ? <div className="serving-role-details"><p className="muted-copy">{row.preference.area.description}</p>
      <ServingFrequencyInput count={draft.frequency_count} label={row.preference.area.name} onChange={(frequency_count, frequency_period) => void persist({ ...draft, frequency_count, frequency_period })} period={draft.frequency_period} /><label>Admin note<textarea value={draft.admin_notes} onBlur={() => void persist(draft)} onChange={(event) => setDraft({ ...draft, admin_notes: event.target.value })} placeholder="Optional note visible to the volunteer" /></label>{row.preference.availability_notes ? <small>Volunteer note: {row.preference.availability_notes}</small> : null}{row.unavailable.length ? <small>Away: {row.unavailable.map((item) => `${item.starts_on}–${item.ends_on}`).join(", ")}</small> : null}
...
function AdminInviteRow({ area, expanded, onChanged, onToggle, userId }: { area: ServingArea; expanded: boolean; onChanged: () => Promise<void>; onToggle: () => void; userId: string }) {
  ...
  return <article className="compact-serving-role"><div className="compact-serving-role-head"><button className="compact-serving-role-main" onClick={onToggle} type="button"><span aria-hidden="true">{expanded ? "▾" : "▸"}</span><span><strong>{area.name}</strong></span></button><button className="text-button compact-role-action" disabled={saving} onClick={() => void save()} type="button">{saving ? "Sending…" : "Invite"}</button></div>{expanded ? <div className="serving-role-details"><p className="muted-copy">{area.description}</p><ServingFrequencyInput count={draft.count} label={area.name} onChange={(count, period) => setDraft({ ...draft, count, period })} period={draft.period} /></div> : null}</article>;
}
