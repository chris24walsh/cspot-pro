...
export interface Member {
  ...
  serving_rotation_modes: Record<string, VolunteerRotationMode>;
...
export async function updateOwnServingSuspension(areaKey: string, suspended: boolean): Promise<VolunteerPreference> { return sendJson<VolunteerPreference>(`/api/v1/identity/serving/preferences/${areaKey}/suspension`, "PATCH", { suspended }); }
export async function addVolunteerUnavailability(payload: { starts_on: string; ends_on: string; note: string | null; role_keys: string[] | null }): Promise<VolunteerUnavailability> { return sendJson<VolunteerUnavailability>("/api/v1/identity/serving/unavailability", "POST", payload); }
...
export async function getUserUnavailability(userId: string): Promise<VolunteerUnavailability[]> { return getJson<VolunteerUnavailability[]>(`/api/v1/identity/serving/admin/users/${userId}/unavailability`); }
export async function addUserUnavailability(userId: string, payload: { starts_on: string; ends_on: string; note: string | null; role_keys: string[] | null }): Promise<VolunteerUnavailability> { return sendJson<VolunteerUnavailability>(`/api/v1/identity/serving/admin/users/${userId}/unavailability`, "POST", payload); }
export async function updateUserUnavailability(userId: string, id: string, payload: { starts_on: string; ends_on: string; note: string | null; role_keys: string[] | null }): Promise<VolunteerUnavailability> { return sendJson<VolunteerUnavailability>(`/api/v1/identity/serving/admin/users/${userId}/unavailability/${id}`, "PATCH", payload); }
export async function removeUserUnavailability(userId: string, id: string): Promise<void> { return deleteRequest(`/api/v1/identity/serving/admin/users/${userId}/unavailability/${id}`); }
...
export async function reviewVolunteerPreference(id: string, payload: { status: VolunteerStatus; preferred_frequency?: VolunteerFrequency; frequency_count?: number; frequency_period?: VolunteerFrequencyPeriod; rotation_mode?: VolunteerRotationMode; admin_notes?: string | null }): Promise<VolunteerPreference> { return sendJson<VolunteerPreference>(`/api/v1/identity/serving/admin/volunteers/${id}`, "PATCH", payload); }
export async function updateAdminServingSuspension(id: string, suspended: boolean): Promise<VolunteerPreference> { return sendJson<VolunteerPreference>(`/api/v1/identity/serving/admin/volunteers/${id}/suspension`, "PATCH", { suspended }); }
